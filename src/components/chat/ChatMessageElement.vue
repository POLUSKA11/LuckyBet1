    ...
    export default {
        ...
        computed: {
            ...
            chatGetGameLink() {
                ...
                if(
                    this.message.message.replace(/&#x2F;/g, '/').match(/(theluckybet\.xyz\/battles\/[0-9a-fA-F]{24})/) !== null ||
                    this.message.message.replace(/&#x2F;/g, '/').match(/(theluckybet\.xyz\/duels\?game=[0-9a-fA-F]{24})$/) !== null
                ) {
                    link = this.message.message.replace(/&#x2F;/g, '/').replace(/.*theluckybet\.xyz/, '');
                }
...
<style scoped>
    ...
    .chat-message-element .element-message.message-partner button.button-user span.user-username {
        background: linear-gradient(255deg, #730d73 0%, #660099 100%), #ffffff;
