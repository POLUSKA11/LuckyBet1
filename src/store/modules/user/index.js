...
const actions = {
    ...
    userTipSocket({ dispatch }, data) {
        dispatch('notificationShow', { type: 'success', message: 'You\'ve been tipped ' + (Math.floor(data.transaction.amount / 10) / 100).toFixed(2) + ' DL.' });
    },
    ...
    userSendUserTipSocket({ getters, commit, dispatch }, data) {
        ...
        getters.socketGeneral.emit('sendUserTip', data, (res) => {
            if(res.success === true) {
                ...
                dispatch('notificationShow', { type: 'success', message: 'You\'ve successfully tipped ' + (Math.floor(res.transaction.amount / 10) / 100).toFixed(2) + ' DL.' });
            } else {
                dispatch('notificationShow', res.error);
            }
